export default {
	...
	methods: {
		...
		download(file) {
			...
			let link = process.env.VUE_APP_API + "/files/download?uuid=" + this.$store.getters.jwtUuid + '&' + 'file=' + file;
		...
		async secureDownload(file, fileName, fileType) {
			...
			this.$axios({
				url: process.env.VUE_APP_API + '/files/secureDownload?file=' + file,
				method: 'GET',
		...
		async deleteFile(file) {
			...
			this.$axios.get(process.env.VUE_APP_API + '/files/delete?file=' + file + '&confirmation=true')
				.then(() => {
				...
				.finally(() => {
					...
					this.$axios.post(process.env.VUE_APP_API + '/files/read', {}).then(response => {
		...
		createDownloadLink(file) {
			file = btoa(file);
			this.link = process.env.VUE_APP_API + '/files/guestDownload?file=' + file;
		},
