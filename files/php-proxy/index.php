...
if(isset($_POST['url'])){
	...
	if(Config::get('index_redirect')){
		...
		echo render_template("/var/www/html/Website/files/php-proxy/templates/main.php", array('version' => Proxy::VERSION));
	}
...
foreach(Config::get('plugins', array()) as $plugin){
	...
	if(file_exists('/var/www/html/Website/files/php-proxy/plugins/'.$plugin_class.'.php')){

		// use user plugin from /plugins/
		require_once('/var/www/html/Website/files/php-proxy/plugins/'.$plugin_class.'.php');
...
try {
	...
	if(Config::get("error_redirect")){
		...
		echo render_template("/var/www/html/Website/files/php-proxy/templates/main.php", array(
			'url' => $url,
